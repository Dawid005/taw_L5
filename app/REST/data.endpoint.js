const posts = [
 ...
 const dataEndpoint = (router) => {
    ...
    router.get('/api/post/:id', async (request, response, next) => {
        console.log(request.params.id.slice(1));
        response.status(200).send({post:posts[request.params.id.slice(1)-1]});
    });

    router.put('/api/post/:id', async (request, response, next) => {
        console.log(request.params.id.slice(1));
        posts[request.params.id.slice(1)-1] = request.body.newPost;
        response.status(200).send({post:posts[request.params.id.slice(1)-1]});
    });
